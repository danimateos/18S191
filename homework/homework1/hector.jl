...
student = (name = "Hector Sanchez", kerberos_id = "jazz")
...
# ╔═╡ 2f56725c-2116-11eb-1b3d-5b69e1e4d75e
length(random_vect)
...
function vecvec_to_matrix(vecvec)
	matrix = zeros(length(vecvec), length(vecvec[1]))
	i = 1
	for vector in vecvec
		matrix[i,:] .= vector
		i += 1
	end
...
function matrix_to_vecvec(matrix)
	vecvec = [matrix[i,:] for i in 1:size(matrix)[1]]
	return vecvec
end
...
function mean_colors(image)
	#result = (mean(channelview(philip)[1,:,:]), mean(channelview(philip)[2,:,:]), mean(channelview(philip)[3,:,:]))
	sum = 0
	sum_r = 0
	sum_g = 0
	sum_b = 0
...
# Here the first option should work, but gives the result without f0 at the end.
...
begin
	...
	function quantize(color::AbstractRGB)
		...
		return RGB(floor(x.r; digits = 1), floor(x.g; digits = 1) ,floor(x.b; digits = 1))
	end
	...
	function quantize(image::AbstractMatrix)
		...
		for i in 1:size(image)[1]
			for j in 1:size(image)[2]
				result[i,j] = RGB(floor(image[i,j].r; digits = 1), floor(image[i,j].g; digits = 1), floor(image[i,j].b; digits = 1))
			end
		end
		return result
	end
...
begin
	...
	function noisify(color::AbstractRGB, s)
		# you will write me in a later exercise!
		red   = clamp(color.r + s*(2*rand()-1), 0, 1)
		green = clamp(color.g + s*(2*rand()-1), 0, 1)
		blue  = clamp(color.b + s*(2*rand()-1), 0, 1)
	...
	function noisify(image::AbstractMatrix, s)
		...
		for i in 1:size(image)[1]
			for j in 1:size(image)[2]
				red   = clamp(image[i,j].r + s*(2*rand()-1), 0, 1)
				green = clamp(image[i,j].g + s*(2*rand()-1), 0, 1)
				blue  = clamp(image[i,j].b + s*(2*rand()-1), 0, 1)
...
# ╔═╡ 37fff618-2b7e-11eb-2273-f94bada24b46
channelview(philip)[1,:,:]

# ╔═╡ 8cc6eba4-23ac-11eb-1ebc-b5b6fd3a2bc4
philip[1,2,1].r
...
# ╔═╡ e64cdf74-2c17-11eb-2207-653f62133e45
length(v)

# ╔═╡ 4e50ae2c-2c18-11eb-36a5-e72142ec8ddc
v[1]
...
# ╔═╡ 86984a00-2c19-11eb-06f4-e1e9716171f6
begin
	v_blur = blur_1D(v, l_box)
	colored_line(v_blur)
end
...
# ╔═╡ 771a47a0-2c1d-11eb-026c-6bbf672da437
begin
...
# ╔═╡ c8e51092-2c1d-11eb-2c81-0b0fa2491cba
extend(v2,2)*k2[2]
...
# ╔═╡ 81b8158e-2c20-11eb-287c-1b3539378b21
sum(v)
...
function gaussian_kernel(n)
	...
	l = (n - 1) ÷ 2
	for x in -l:l
		kernel[x+l+1] = 1/(sqrt(2*pi))*exp((-x^2)/2)
	end
...
gaussian_kernel_size_1D = 7 # change this value, or turn me into a slider!

# ╔═╡ b5c9a172-2c21-11eb-25ca-576833436a3e
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	l_rows = Int((num_rows_k - 1)/2)
	l_cols = Int((num_columns_k - 1)/2)
	new_v = copy(M)
	for i in 1:num_rows_m
		for j in 1:num_columns_m
			reduced_mat = [extend_mat(M, i-x, j-y) for x in -l_rows:l_rows, y in -l_cols:l_cols]
			new_v[i,j] = sum(reduced_mat.*K)
		end
	end
...
# ╔═╡ 14e8193c-2c30-11eb-279b-6997191dc79a
function gaussian_kernel_2D()
	...
	for x in -l:l
		for y in -l:l
			kernel[x+l+1, y+l+1] = 1/(sqrt(2*pi))*exp((-(x^2+y^2))/2)
		end
	end
	kernel = kernel/sum(kernel)
	return kernel
...
# ╔═╡ 88551b36-2c30-11eb-219b-e3f58d82b124
...
# ╔═╡ 3fe5fbf2-2c32-11eb-09cc-8f06f211cc97
typeof(sum(Gray.(philip)[1:3, 1:3].*k1))

# ╔═╡ 9f6cd5fa-2c32-11eb-05e3-1b9c30eac2e4
function convolve_image2(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i in 1:num_rows_m
		for j in 1:num_columns_m
			reduced_mat = [extend_mat(M, i-x, j-y) for x in -l_rows:l_rows, y in -l_cols:l_cols]
			new_v[i,j] = clamp(sum(reduced_mat.*K), 0, 1)
		end
	end
...
function with_sobel_edge_detect(image)
	...
	image2 = channelview(image)[1,:,:]
	
	gx = convolve_image2(image2, k1)
	gy = convolve_image2(image2, k2)
...
# ╔═╡ bdd3ce28-2c31-11eb-0a8f-035fa54c9089
